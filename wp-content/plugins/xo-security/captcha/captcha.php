...
// Prefix.
$prefix = isset( $_GET['prefix'] ) && preg_match( '/^[a-zA-Z0-9]{1,20}$/ ', $_GET['prefix'] ) ? $_GET['prefix'] : 'default'; // phpcs:ignore Standard.Category.SniffName.ErrorCode, WPCS: input var ok, CSRF ok, sanitization ok.

// 文字モード（'en' または 'jp'）.
$char_mode = isset( $_GET['char_mode'] ) && in_array( $_GET['char_mode'], array( 'en', 'jp' ), true ) ? $_GET['char_mode'] : 'en'; // phpcs:ignore Standard.Category.SniffName.ErrorCode, WPCS: input var ok, CSRF ok, sanitization ok.
...
if ( 'jp' === $char_mode ) {
	// 文字列テーブル (判別しにくい「く」「へ」「ぬ」「ね」「る」を除外).
	$string_table = 'あいうえおかきけこさしすせそたちつてとなにのはひふほまみむめもやゆよらりれろわをん';
	// 基準のフォント サイズ.
...
for ( $i = 0; $i < $string_len; $i++ ) {
	$x = (int) ( ( ( $captcha_image_width - 8 ) / mb_strlen( $string ) ) * $i + mt_rand( (int) ( -$captcha_font_size / 10 ), (int) ( $captcha_font_size / 10 ) ) + 4 );
