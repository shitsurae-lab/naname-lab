...
?>
  ...
  elseif (is_object_in_term($post->ID, 'achievement_cat', ['containing-ecommerce', 'containing-woocommerce', 'containing-wordpress', 'containing-landing-page', 'containing-banner']) && is_singular('achievement')) :
    $terms = get_the_terms($post->ID,  'achievement_cat');

    foreach ($terms as $term) {
      $term_name = $term->name;
      $term_name = str_replace('Containing ', '', $term_name);
      $term_slug = $term->slug;
